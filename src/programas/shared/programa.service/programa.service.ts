...
export class ProgramaService {
  ...
  async requisitarDadosESalvarNoBD(data: string): Promise<number> {
    try {
      const verificarProgramacao = await this.prisma.programme.findUnique({
        where: {
          date: data,
        },
      });

      if (verificarProgramacao) {
        throw new Error('A programação desta data já existe');
      } else {
        const id = 1337;
            ...
            data: {
              ...
              Resumo: {
                create: {
                  ...
                  ResumoImprensa:
                    dadosCustomInfo?.Resumo?.ResumoImprensa || null,
